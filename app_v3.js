// ==========================================================================
// CONTROLADOR PRINCIPAL E ROTEADOR SPA CLÁSSICO (app_v3.js)
// ==========================================================================
...
async function checkAndInitDatabase() {
  ...
  if (missing) {
    try {
      ...
      console.error("Erro ao ler dados mockados via fetch, utilizando dados estáticos embutidos de emergência:", err);
      
      const fallbackGroups = [{ id: "g_1", nome: "Quarta Show Society" }];
      const fallbackPlayers = [
        { id: "p_1", nome: "Carlos Henrique", data_nascimento: "1994-05-12", cpf: "111.111.111-11", whatsapp: "(11) 99999-9999", goleiro: false, autoavaliacao: 4, ativo: true, saldo: 25.00, gols: 12, partidas: 15, avaliacao_media: 4.1 },
        { id: "p_2", nome: "Marcos Goleiro", data_nascimento: "1990-08-20", cpf: "222.222.222-22", whatsapp: "(11) 88888-8888", goleiro: true, autoavaliacao: 3, ativo: true, saldo: -10.00, gols: 0, partidas: 14, avaliacao_media: 3.2 }
      ];
      const fallbackPeladas = [
        { id: "pel_1", grupo_id: "g_1", data: "2026-07-22", horario: "20:00", status: "agendada", valor_convocacao: 20.00 }
      ];
      const fallbackConfigs = [
        { grupo_id: "g_1", criterio_empate: "ambos_permanecem", vitorias_para_sair: 2, jogadores_por_time: 6, quantidade_times: 3, regra_saida: "final_fila", valor_convocacao: 20.00, limite_saldo_negativo: 100.00 }
      ];
      
      localStorage.setItem("groups", JSON.stringify(fallbackGroups));
      localStorage.setItem("players", JSON.stringify(fallbackPlayers));
      localStorage.setItem("peladas", JSON.stringify(fallbackPeladas));
      localStorage.setItem("configs", JSON.stringify(fallbackConfigs));
...
function initAuthEvents() {
  ...
  document.getElementById("btn-quick-player").onclick = () => {
    ...
    window.App.currentUser = players[0] || { id: "p_1", nome: "Carlos Henrique", autoavaliacao: 4, goleiro: false, saldo: 25.00 };
