    ...
    if ((peladaAtiva.modo === 'torneio' || tState) && window.TournamentEngine && tState) {
      ...
      if (currentMatchObj && currentMatchObj.status !== 'encerrado') {
        ...
        const getTeamId = (t) => {
          if (!t) return '';
          if (typeof t === 'object') {
            return String(t.id || t.team_id || t.nome || t.name || '').trim().toLowerCase();
          }
          return String(t).trim().toLowerCase();
        };

        const liveMatchA = window.App.liveMatch ? (window.App.liveMatch.teamAObj || window.App.liveMatch.teamA) : null;
        const liveMatchB = window.App.liveMatch ? (window.App.liveMatch.teamBObj || window.App.liveMatch.teamB) : null;

        const liveIdA = getTeamId(liveMatchA);
        const liveIdB = getTeamId(liveMatchB);
        const schedIdA = getTeamId(currentMatchObj.teamAObj || currentMatchObj.teamA);
        const schedIdB = getTeamId(currentMatchObj.teamBObj || currentMatchObj.teamB);

        // Verifica por ID/identificador se os lados dos times no placar ao vivo estão invertidos em relação ao jogo agendado
        const isFlipped = (
        ...
        if (isFlipped) {
          currentMatchObj.golsA = scoreB;
          currentMatchObj.golsB = scoreA;
        } else {
          currentMatchObj.golsA = scoreA;
          currentMatchObj.golsB = scoreB;
        }
        ...
        if (currentMatchObj.penaltisA !== undefined && currentMatchObj.penaltisA !== null) {
          ...
          currentMatchObj.vencedor = null; // Empate: sem vencedor na tabela de grupos!
        }
      }
